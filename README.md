...
服务仅监听 `127.0.0.1`。登录态保存在本机 `.state` 目录，不会因启动本地服务而自动上传到 VPS。
...
`PRIVATE_SYSU_SINGLE_USER_FALLBACK` 必须保持为 `0`，否则不同本地会话可能错误复用最近用户的校园登录态。
...
- 不要提交 `.env`、`.state`、Cookie、Bearer token、二维码或校园账号密码。
- 不要将本项目直接监听到 `0.0.0.0` 后暴露到公网。
- 本地授权状态具有访问个人校园信息的能力，应按密码同等级别保护。
- 退出或转交电脑前，删除 `.state` 可以清除本项目保存的本地登录态。

## SYSU-Anything 的关系

本项目使用 SYSU-Anything 作为校园系统驱动层，负责 CAS/企业微信登录、会话持久化、教务连接以及逸问发送和历史记录操作。它是当前实现依赖，不是学校官方要求安装的软件。

- [SYSU-Anything](https://github.com/qybaihe/SYSU-Anything)

本仓库通过 `package.json` 固定并自动安装该依赖，因此普通用户只需运行 `setup.ps1`。
